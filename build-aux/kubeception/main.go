...
import (
	...
	buildaux "github.com/datawire/ambassador-agent/build-aux"
)
...
func run() error {
	...
	token, ok := os.LookupEnv("DEV_AMBASSADOR_AGENT_KUBECEPTION_TOKEN")
	if !ok {
		return fmt.Errorf("Please set the DEV_AMBASSADOR_AGENT_KUBECEPTION_TOKEN environment variable to a valid kubeception token")
	}
	cli := &http.Client{}
	switch verb {
	case "create":
		...
		defer cancel()
		err := buildaux.Retry(ctx, "kubeception", func(ctx context.Context) error {

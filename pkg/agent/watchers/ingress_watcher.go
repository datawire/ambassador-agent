...
func isNetworkingAPIAvailable(ctx context.Context, clientset kubernetes.Interface, namespace string) bool {
	if namespace == "" {
		namespace = "default"
	}
...
func getIngressWatcher(ctx context.Context, namespaces []string, cond *sync.Cond, om ObjectModifier) ingressWatcher {
	...
	if isNetworkingAPIAvailable(ctx, k8sif, namespaces[0]) {
